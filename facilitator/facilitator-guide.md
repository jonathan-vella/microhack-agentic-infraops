...
| **Copilot access issue** | Agent picker is empty; "Copilot is not available"; custom agents missing | Verify participant has a Copilot Business or Enterprise license — other SKUs do not include the required functionality ([plans](https://github.com/features/copilot/plans)). Check `customAgentInSubagent.enabled` setting. Reload VS Code window. | If license is wrong tier, participant cannot use custom agents. Pair with another team member who has access. |
